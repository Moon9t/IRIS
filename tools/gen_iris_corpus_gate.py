...
HEADER = '''//! The `.iris` corpus: executed, and gated on asserting its results.
//!
...
//!
//! **Most files assert nothing.** MISSING_N of TOTAL_N print results without
...
io.open("tests/iris_corpus.rs", "w", encoding="utf-8", newline="").write(src)
print("wrote tests/iris_corpus.rs: needs=%d total=%d known_broken=%d must_fail=%d"
      % (len(missing), len(tests), len(KNOWN_BROKEN), len(MUST_FAIL)))
