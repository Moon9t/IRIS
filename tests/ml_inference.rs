...
mod ml_inference_tests {
    ...
    fn test_ml_integration_bridge_ready() {
        ...
        println!("  ONNXRUNTIME_DIR: {}", 
            if is_onnx_enabled { "✓ SET" } else { "✗ NOT SET" });
        println!("  Test models: {}",
            if fixtures_exist { "✓ FOUND" } else { "✗ NOT FOUND" });
        println!();
        ...
        if is_onnx_enabled && fixtures_exist {
            ...
            if !fixtures_exist {
                println!("  {}️⃣  Generate test models", if is_onnx_enabled { "1" } else { "5" });
                println!("     cd tests");
    ...
    fn test_tensor_bridge_layer_structure() {
        ...
        println!("  ✓ Input elements: {} = product of shape dimensions", numel);
    ...
    fn test_ml_framework_status() {
        ...
        let libtorch_enabled = std::env::var("LIBTORCH_DIR").is_ok();
        let tensorflow_enabled = std::env::var("TENSORFLOW_DIR").is_ok();

        println!("Framework availability:");
        println!("  ONNX Runtime:  {} {}",
            if onnx_enabled { "✓" } else { "✗" },
            if onnx_enabled { "(enabled)" } else { "(configure: set ONNXRUNTIME_DIR)" });
        
        println!("  LibTorch:      {} {}",
            if libtorch_enabled { "✓" } else { "✗" },
            if libtorch_enabled { "(enabled)" } else { "(configure: set LIBTORCH_DIR)" });
        
        println!("  TensorFlow:    {} {}",
            if tensorflow_enabled { "✓" } else { "✗" },
            if tensorflow_enabled { "(enabled)" } else { "(configure: set TENSORFLOW_DIR)" });
        println!();
