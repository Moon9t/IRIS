...
fn test_tensorrt_engine_compilation() {
    ...
    assert!(
        res.is_ok(),
        "TensorRT engine compilation failed: {:?}",
        res.err()
    );
    ...
    assert!(
        trt_out.contains("NVIDIA TensorRT"),
        "Expected TensorRT header reference"
    );
    assert!(
        trt_out.contains("tensorrt_add_gemm"),
        "Expected Gemm layer lowering"
    );
    assert!(
        trt_out.contains("deserialize_cuda_engine"),
        "Expected engine deserialization FFI call"
    );
    assert!(
        trt_out.contains("execution_context_enqueue_v3"),
        "Expected async JIT execution FFI call"
    );
}
