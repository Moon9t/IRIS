...
Download `IRIS-0.3.0-windows-x64-setup.exe` from the
...
```bash
sudo dpkg -i iris_0.3.0_x86_64.deb
# or
sudo apt install ./iris_0.3.0_x86_64.deb
```

**Fedora/RHEL (.rpm):**

```bash
sudo rpm -i iris-0.3.0-1.x86_64.rpm
# or
sudo dnf install ./iris-0.3.0-1.x86_64.rpm
```

**AppImage (portable, no root):**

```bash
chmod +x IRIS-0.3.0-x86_64.AppImage
./IRIS-0.3.0-x86_64.AppImage --version
```
...
```bash
sudo installer -pkg IRIS-0.3.0-macos-aarch64.pkg -target /
```
...
1. Download `IRIS-0.3.0-windows-x64-setup.exe` from the latest release
...
```powershell
Expand-Archive IRIS-0.3.0-windows-x86_64-portable.zip -DestinationPath C:\iris
$env:PATH += ";C:\iris"
```
...
Options:
- `--version 0.3.0` — set the version (default: 0.3.0)
...
**Linux .deb:**
```bash
bash installer/linux/build-deb.sh --version 0.3.0 --arch x86_64
```

**Linux .rpm:**
```bash
bash installer/linux/build-rpm.sh --version 0.3.0 --arch x86_64
```

**Linux AppImage:**
```bash
bash installer/linux/build-appimage.sh --version 0.3.0
```

**macOS .pkg:**
```bash
bash installer/macos/build-pkg.sh --version 0.3.0
```

**macOS .dmg:**
```bash
bash installer/macos/build-dmg.sh --version 0.3.0
```
...
**`clang not found`** — `iris run`, `iris build`, `--emit eval`, and `--emit jit`
use the LLVM/native pipeline. Install clang and the target sysroot/linker:

```bash
sudo apt install clang lld         # Debian/Ubuntu
sudo dnf install clang lld         # Fedora
brew install llvm                  # macOS
# Windows: use the Full installer, which bundles LLVM and MinGW UCRT64
```

See [../docs/current-language-lock.md](../docs/current-language-lock.md#binary-install-dependencies)
for the full binary dependency matrix and bundling policy.
