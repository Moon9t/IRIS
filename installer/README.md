...
Download `IRIS-0.6.1-windows-x64-setup.exe` from the
...
```bash
sudo dpkg -i iris_0.6.1_x86_64.deb
# or
sudo apt install ./iris_0.6.1_x86_64.deb
```

**Fedora/RHEL (.rpm):**

```bash
sudo rpm -i iris-0.6.1-1.x86_64.rpm
# or
sudo dnf install ./iris-0.6.1-1.x86_64.rpm
```
...
```bash
sudo installer -pkg IRIS-0.6.1-macos-aarch64.pkg -target /
```
...
1. Download `IRIS-0.6.1-windows-x64-setup.exe` from the latest release
...
```powershell
Expand-Archive IRIS-0.6.1-windows-x86_64-portable.zip -DestinationPath C:\iris
$env:PATH += ";C:\iris"
```
...
Options:
- `--version 0.6.1` — set the version (default: 0.6.1)
...
**Linux .deb:**
```bash
bash installer/linux/build-deb.sh --version 0.6.1 --arch x86_64
```

**Linux .rpm:**
```bash
bash installer/linux/build-rpm.sh --version 0.6.1 --arch x86_64
```

**Linux AppImage:**
```bash
bash installer/linux/build-appimage.sh --version 0.6.1
```

**macOS .pkg:**
```bash
bash installer/macos/build-pkg.sh --version 0.6.1
```

**macOS .dmg:**
```bash
bash installer/macos/build-dmg.sh --version 0.6.1
```
