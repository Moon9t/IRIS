...
pub fn emit_tensorrt(module: &IrModule) -> Result<String, CodegenError> {
    ...
    writeln!(
        out,
        "; Target: NVIDIA TensorRT Engine (ICudaEngine) via FFI"
    )?;
    ...
    writeln!(
        out,
        "; ── TensorRT Engine Compilation Plan ──────────────────────────────────"
    )?;
                ...
                } = instr
                {
                    ...
                    match op {
                        TensorOp::Einsum { notation } => {
                            writeln!(
                                out,
                                "  Layer {}: tensorrt_add_gemm [Einsum: {}]",
                                layer_count, notation
                            )?;
                        }
                        TensorOp::Unary { op } => {
                            writeln!(
                                out,
                                "  Layer {}: tensorrt_add_activation [Op: {}]",
                                layer_count, op
                            )?;
                        }
                        TensorOp::Reshape => {
                            writeln!(
                                out,
                                "  Layer {}: tensorrt_add_shuffle [Reshape]",
                                layer_count
                            )?;
                        }
                        TensorOp::Transpose { axes } => {
                            writeln!(
                                out,
                                "  Layer {}: tensorrt_add_shuffle [Transpose: {:?}]",
                                layer_count, axes
                            )?;
                        }
    ...
    writeln!(
        out,
        "; ── FFI Host Declarations for Execution ──────────────────────────────"
    )?;
    writeln!(
        out,
        "extern \"C\" fn deserialize_cuda_engine(bytes: *const u8, size: usize) -> *mut c_void;"
    )?;
    writeln!(out, "extern \"C\" fn execution_context_enqueue_v3(ctx: *mut c_void, inputs: *mut *mut f32, outputs: *mut *mut f32) -> bool;")?;
    writeln!(
        out,
        "extern \"C\" fn tensorrt_add_gemm(network: *mut c_void) -> *mut c_void;"
    )?;
    writeln!(out)?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "; Status: NVIDIA TensorRT Engine lowered successfully."
    )?;
